...
class YouTubeDataCollector:
    ...
    def get_video_details(self, driver, video_url):
        """
        Loads a YouTube video page and extracts basic details: title, view count, and comment count.
        """
        ...
        return {"url": video_url, "title": title, "views": views, "comments": comments}
    ...
    def get_stream_details(self, driver, video_url):
        """
        Loads a live stream video page and extracts details: title, view count, live status, and scheduled time.
        """
        driver.get(video_url)
        time.sleep(4)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        ...
        return {"url": video_url, "title": title, "views": views, "live_status": live_status, "scheduled": scheduled}
    ...
    def get_shorts_details(self, driver, video_url):
        """
        Loads a Shorts video page and extracts details: title, view count, and comment count.
        """
        driver.get(video_url)
        time.sleep(4)
        soup = BeautifulSoup(driver.page_source, 'html.parser')
